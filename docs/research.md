...
This document describes research work that was explored in jaxboost but has been removed to focus the library on its core value proposition: automatic objective functions for XGBoost/LightGBM.

The code for these features has been archived. If you're interested in any of these directions, please open an issue.
...
| Split | Description |
|-------|-------------|
| `HyperplaneSplit` | Linear combination of features: `w · x ≤ t` |
| `AxisAlignedSplit` | Single feature threshold (like traditional trees): `x[j] ≤ t` |
...
```
Loss = -I(Y; Z) + β · I(X; Z)
     ≈ Prediction_Loss + β · KL[p(z|x) || p(z)]
```

Key insight: Trees are information channels. Each tree compresses input X into leaf assignment Z, then predicts Y from Z.
...
Instead of discrete boosting rounds, solve an ODE where the tree output is the velocity field. Traditional boosting with learning rate η is equivalent to Euler discretization with step size η.
...
If there's interest in reviving any of these features, please open an issue on GitHub.
