...
def log_loss_metric(transform: Callable[[np.ndarray], np.ndarray] | None = None, eps: float = 1e-7) -> Metric:
    """
...
def accuracy_metric(threshold: float = 0.5, transform: Callable[[np.ndarray], np.ndarray] | None = None) -> Metric:
    """
    ...
    def _transform(predt: np.ndarray) -> np.ndarray:
        if transform is not None:
            probs = transform(predt)
        else:
            probs = _sigmoid(predt)
...
def f1_metric(threshold: float = 0.5, transform: Callable[[np.ndarray], np.ndarray] | None = None) -> Metric:
    """
...
def precision_metric(threshold: float = 0.5, transform: Callable[[np.ndarray], np.ndarray] | None = None) -> Metric:
    """
...
def recall_metric(threshold: float = 0.5, transform: Callable[[np.ndarray], np.ndarray] | None = None) -> Metric:
    """
